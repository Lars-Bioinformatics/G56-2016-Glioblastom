...
configfile: "/work/sduvarcall/G56-2016-Glioblastom/WES/samples_wes.yaml"
...
funcotator_somatic = "/work/sduvarcall/Resources/funcotator/funcotator_dataSources.v1.6.20190124.somatic/"
# Panel of normals location
pon_location = "somatic_panel_of_normals/"

# Sample information
# PAIR, = glob_wildcards("{sample}.recalibrated.bam")
...
rule all_pairs:
    input:
        # [expand(output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag.vcf.gz",
        [expand(output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag_annotated_PASSonly.vcf.gz",
...
rule Mutect2_tumor_only_pon:
    input:
        bam="bam/{sample}.recalibrated.bam",
...
rule Mutect2_matched:
    input:
        normal="bam/{normal}.recalibrated.bam",
        tumor="bam/{tumor}.recalibrated.bam",
        pon=pon_location+"pon.vcf.gz",
        intervals=target_regions+"{interval}.interval_list"
    output:
...
rule FilterMutectCalls:
    ...
    output:
        ...
        vcf_pass=output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag_PASSonly.vcf.gz",
...
#########################################################
####             Annotate Mutect2 Calls              ####
#########################################################
rule funcotator:
    input:
        vcf=output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag_PASSonly.vcf.gz",
    output:
        vcf=output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag_annotated_PASSonly.vcf.gz",
    params:
        vcf=output_somatic+"{tumor}_vs_{normal}_somatic_mutect2_filterFlag_annotated_PASSonly.vcf"
    threads: 1
    shell:
        """
        gatk Funcotator \
             ...
             --reference {ref} \
             --ref-version hg38 \
             --data-sources-path {funcotator_somatic} \
             --output {params.vcf} \
